...
<Warning>
  The Legacy Partner API is deprecated and no longer in service. Do not use it. This reference is
  kept only to help you move an existing integration to the [New Partner API](/api/partner-api).
</Warning>

The Legacy Partner API assists organizations that use the Cyrisma risk management platform to further automate and enhance overall risk management.
...
<Card title="Note on API URLs" type="note">
  The central API URL **https://api.cyrisma.com/app** is used for login, token generation, provisioning, and partner/instance management. Scan and reporting endpoints require an additional instance authentication step and are called against an instance-specific URL in the format [**https://ccnnnnnn.cyrisma.com/app**](https://ccnnnnnn.cyrisma.com/app) (e.g. **https://ccnnnnnn.cyrisma.com/app/partner/scans/data**).
</Card>
...
<Card title="New Partner API" icon="arrow-right" href="/api/partner-api">
  The replacement for this API. It covers the same provisioning and reporting areas against the
  current Cyrisma platform, uses JSON, and is served entirely from your instance host.
</Card>