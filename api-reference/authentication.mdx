...
If instance\_id is not provided, all available CYRISMA instances are authenticated at once. The response returns an instance\_url value for each authenticated instance. This instance\_url is then used to build all reporting endpoint URLs using the format:
