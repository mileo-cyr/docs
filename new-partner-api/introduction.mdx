...
| Credentials | API Key and Secret from Cyrisma Support | Self-managed machine clients |
...
<Steps>
  <Step title="Create a machine client">
    Call `POST /ta/v1/partners/machine-clients` to create a credential. The response contains the
    `client_id` and the `client_secret`. The secret is returned once and cannot be retrieved again.
  </Step>
